...
function handle_delete_popup() {
  let type_name = document.getElementById("tag_name");
  ...
  const openPopup = (event) => {
    ...
    type_id = event.target.getAttribute("tag_id");
    type_name.innerHTML = event.target.getAttribute("tag_name");
  ...
  confirmBtn.addEventListener("click", async () => {
    try {
      const response = await fetch(`/api/types/${type_id}`, {
        method: "DELETE",
        headers: {
...
function handle_form_popup() {
  ...
  editBtns.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      type_id = event.target.getAttribute("tag_id");
      openEditPopup(type_id);
    });
  });

  const openEditPopup = (type_id) => {
    initialize_edit_form(type_id);
    formPopupOverlay.classList.add("visible");
  };

  const initialize_edit_form = (type_id) => {
    fetch(`/api/types/${type_id}/`)
      .then((response) => {
  ...
  const submit_form = async () => {
    let isEdit = type_id ? true : false;
    let url = `/api/types/`;
    if (isEdit) {
      url = `/api/types/${type_id + `/`}`;
    }
