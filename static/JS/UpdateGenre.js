...
function handle_delete_popup() {
  let genre_name = document.getElementById("tag_name");
  ...
  const openPopup = (event) => {
    ...
    genre_id = event.target.getAttribute("tag_id");
    genre_name.innerHTML = event.target.getAttribute("tag_name");
  ...
  confirmBtn.addEventListener("click", async () => {
    try {
      const response = await fetch(`/api/genres/${genre_id}`, {
        method: "DELETE",
        headers: {
...
function handle_form_popup() {
  ...
  editBtns.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      genre_id = event.target.getAttribute("tag_id");
      openEditPopup(genre_id);
    });
  });

  const openEditPopup = (genre_id) => {
    initialize_edit_form(genre_id);
    formPopupOverlay.classList.add("visible");
  };

  const initialize_edit_form = (genre_id) => {
  ...
  const submit_form = async () => {
    let isEdit = genre_id ? true : false;
    let url = `/api/genres/`;
    if (isEdit) {
